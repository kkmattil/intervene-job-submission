...
#print("look for biobank specific requests")
for j in all_objs:
...
#1print ("check status of tasks")
status_levels=["submitted", "processing", "ready" ]
for task in my_tasks:
    ...
    print(task+ " "+status_levels[task_stat_num])
...
#read the description of selected job
object_contents=conn.get_object(bucket, biobank+"/requests/"+target_task, headers=None)[1]
...
if operation == "download":
   ...
   for input_file in task_description["inputs"]:
       #print("Downloading "+bucket+input_file["url"]+" to "+target_task+input_file["path"])
       download_large_file(bucket, input_file["object"], target_task+"/"+input_file["path"])
   
   for input_file in task_description["instructions"]:
       download_large_file(bucket, input_file["object"], target_task+"/README_instructions")
